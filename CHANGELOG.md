...
- **悬浮提示显示「设备在线 / 设备离线」**：在 Jade / WTHIT（以及 AE2 自带的悬浮提示）里与 AE2 自家机器一致。
  走 AE2 的公开扩展点 `appeng.api.integrations.igtooltip.TooltipProvider` + `META-INF/services` 接入，
  不引入 Jade 编译期依赖，也不自己写 Jade 插件。
